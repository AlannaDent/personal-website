...
Nothing here is Relativity work.
...
- The GitHub command-line tool (`gh`) on this Mac may be signed in to both accounts. Before
  any GitHub action, run `gh auth status` and make sure **AlannaDent** is the active account.
- When this folder becomes a git repository, set the git name and email **for this
  folder only** to my personal account, so commits here never carry my work identity.
...
- This rule exists to avoid mixing up my two GitHub accounts.
